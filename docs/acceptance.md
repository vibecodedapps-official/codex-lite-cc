...
8. **Allow rules match the installed version.** Run `/codex-lite:setup` and check that each
   allow rule it prints matches the paths of the version actually installed, with a `*` in
   place of the version number in the plugin path. Paste them into the scratch settings as
   printed and confirm they are valid JSON and match.
