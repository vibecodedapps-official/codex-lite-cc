...
| 2026-09-25 | 0.3.0 | macOS 27.0, Claude Code 2.1.280, headless `claude -p --permission-mode default` with `--plugin-dir` from a scratch git repository, `CODEX_LITE_CODEX_BIN` set to the test suite's fake Codex | fake Codex only | Item 11 in part, the routing half. With the 0.2.1 command files, "dispatch codex to review my uncommitted changes" invoked no plugin command and Claude ran `codex exec` directly. With the 0.3.0 files, the same prompt invoked `/codex-lite:review` through the Skill tool; in default mode that invocation was itself a prompt ("Execute skill: codex-lite:review"), denied headless. With a `Skill(codex-lite:review)` allow rule the command body loaded, Claude read the request file, then wrote it with the empty argument text, and the Write was blocked as a sensitive file as in the 0.2.1 row, so the Bash step was not reached. The `ask` half, the `do` refusal, and the `--base main` argument check were not run. `npm test` (89 pass, 9 skipped) and `npm run lint` pass. Items 1 to 10 and 12 to 15 not run: this row needs an interactive session and the real CLI before release. |
